...
public float zoom = 0.25f; // чувствительность при увеличении,
...
-Mathf.Abs(zoomMax)/2); // начальная позиция камеры
start_values = true;
}
...
else if(Input.GetAxis(&quot;Mouse ScrollWheel&quot;) &lt; 0) offset.z -=

zoom;

offset.z = Mathf.Clamp(offset.z, -Mathf.Abs(zoomMax), -
Mathf.Abs(zoomMin));

74

X = transform.localEulerAngles.y + Input.GetAxis(&quot;Mouse X&quot;)
* sensitivity; // смена позиции по координате X
